...
export class IntelligentRepoAnalyzer {
  ...
  private maxFilesToAnalyze = 50;
  private maxFileSize = 50000; // 50KB
  ...
  private async analyzeStructure() {
    ...
    let codebaseSize: 'small' | 'medium' | 'large' | 'huge';
    if (totalFiles < 50) codebaseSize = 'small';
    else if (totalFiles < 200) codebaseSize = 'medium';
    else if (totalFiles < 1000) codebaseSize = 'large';
    else codebaseSize = 'huge';
  ...
  private async detectTechnology(structure: any) {
    const technology = {
  ...
  private async analyzePatterns(structure: any, technology: any) {
    ...
    if (structure.directories.some((d: string) => d.includes('components'))) {
      patterns.architecture = 'Component-Based';
      patterns.designPatterns.push('Component Pattern');
    }

    if (structure.directories.some((d: string) => d.includes('services'))) {
      patterns.designPatterns.push('Service Layer');
    }

    if (structure.directories.some((d: string) => d.includes('controllers'))) {
      patterns.architecture = 'MVC';
      patterns.designPatterns.push('MVC Pattern');
    }

    // Detect testing strategy
    if (structure.keyFiles.some((f: string) => f.includes('test') || f.includes('spec'))) {
      patterns.testingStrategy = 'Unit Testing';
    }

    if (structure.keyFiles.some((f: string) => f.includes('jest.config') || f.includes('vitest.config'))) {
  ...
  private async assessComplexity(structure: any, technology: any, dependencies: any) {
  ...
  private async understandContext(structure: any, technology: any, dependencies: any) {
    const context = {
      projectType: 'Unknown',
      developmentStage: 'Unknown',
      teamSize: 'unknown' as 'solo' | 'small' | 'medium' | 'large' | 'unknown',
      documentation: 'poor' as 'poor' | 'basic' | 'good' | 'excellent'
    };

    // Detect project type
    if (technology.frameworks.some((f: string) => ['React', 'Vue.js', 'Angular'].includes(f))) {
      context.projectType = 'Frontend Application';
    } else if (technology.frameworks.some((f: string) => ['Express.js', 'NestJS'].includes(f))) {
      context.projectType = 'Backend API';
    } else if (technology.frameworks.some((f: string) => ['Next.js', 'Nuxt.js'].includes(f))) {
      context.projectType = 'Full-Stack Application';
    }

    // Assess documentation
    const hasReadme = structure.keyFiles.some((f: string) => f.toLowerCase().includes('readme'));
    const hasChangelog = structure.keyFiles.some((f: string) => f.toLowerCase().includes('changelog'));
    const hasDocs = structure.directories.some((d: string) => d.includes('docs'));
  ...
  private async basicAnalysis(): Promise<RepoAnalysis> {
    const structure = await this.analyzeStructure();
    const technology = await this.detectTechnology(structure);
