...
async function registerServer(): Promise<void> {
  const answers = await inquirer.prompt([
    ...
    },
    {
      ...
      name: 'type',
      message: 'Server type:',
      choices: ['filesystem', 'github', 'slack', 'claude-flow', 'custom']
    },
    {
      type: 'input',
      name: 'command',
    ...
    },
    {
      type: 'input',
      name: 'capabilities',
      message: 'Server capabilities (comma-separated):',
      filter: (input) => input ? input.split(',').map((s: string) => s.trim()) : []
    }
  ]);
  ...
  try {
    const result = await mcpClient.registerServer({
      ...
      capabilities: answers.capabilities.length > 0 ? answers.capabilities : undefined
    });
    
    console.log(colors.success(`✅ Server registered successfully!`));
    console.log(colors.info(`Server ID: ${result.serverId}`));
  } catch (error) {
    console.error(colors.error('❌ Failed to register server:'), error);
    process.exit(1);
  }
}
...
${colors.info('Commands:')}
  ...
  graphyn mcp register                 Register a new MCP server
