...
import type { EnhancedUXConfig } from './types.js';
...
export class EnhancedUXCLI {
  private orchestrator: SplitScreenOrchestrator | null = null;
  ...
  private setupEventHandlers(): void {
    ...
    this.orchestrator.on('performance_warning', (warning: string) => {
      if (this.config.features.enablePerformanceMonitoring) {
 ...
 */
export function parseEnhancedUXArgs(args: string[]): CLIOptions {
 ...
 */
export async function runEnhancedUXCLI(args: string[] = process.argv.slice(2)): Promise<void> {
