...
export class RepositoryContextManager extends EventEmitter {
     ...
     */
    async analyzeRepositoryStructure(repositoryPath) {
        const repository = {
            path: repositoryPath,
     ...
     */
    async detectNodeStack(repositoryPath, repository) {
        try {
            ...
            for (const lockFile of lockFiles) {
                try {
                    ...
                    if (lockFile.includes('pnpm'))
                        repository.packageManagers.push('pnpm');
                    else if (lockFile.includes('yarn'))
                        repository.packageManagers.push('yarn');
                    else if (lockFile.includes('bun'))
                        repository.packageManagers.push('bun');
                    else
            ...
            // Detect frameworks
            if (allDeps.react)
                repository.frameworks.push('react');
            if (allDeps['next'] || allDeps['@next/core'])
                repository.frameworks.push('next.js');
            if (allDeps.vue)
                repository.frameworks.push('vue');
            if (allDeps.svelte)
                repository.frameworks.push('svelte');
            if (allDeps.express)
                repository.frameworks.push('express');
            if (allDeps.fastify)
                repository.frameworks.push('fastify');
            if (allDeps['@nestjs/core'])
                repository.frameworks.push('nestjs');
        }
        catch (error) {
     ...
     */
    async detectRustStack(repositoryPath, repository) {
        try {
            ...
            if (cargoContent.includes('tokio'))
                repository.frameworks.push('tokio');
            if (cargoContent.includes('axum'))
                repository.frameworks.push('axum');
            if (cargoContent.includes('actix'))
                repository.frameworks.push('actix');
            if (cargoContent.includes('warp'))
                repository.frameworks.push('warp');
        }
        catch (error) {
     ...
     */
    async detectPythonStack(repositoryPath, repository) {
        try {
            ...
            if (hasPython) {
                ...
                try {
                    ...
                    if (requirements.includes('django'))
                        repository.frameworks.push('django');
                    if (requirements.includes('flask'))
                        repository.frameworks.push('flask');
                    if (requirements.includes('fastapi'))
                        repository.frameworks.push('fastapi');
                    if (requirements.includes('streamlit'))
                        repository.frameworks.push('streamlit');
                }
                catch {
     ...
     */
    async detectGoStack(repositoryPath, repository) {
        try {
            ...
            if (goModContent.includes('gin-gonic'))
                repository.frameworks.push('gin');
            if (goModContent.includes('echo'))
                repository.frameworks.push('echo');
            if (goModContent.includes('fiber'))
                repository.frameworks.push('fiber');
        }
        catch (error) {
     ...
     */
    async analyzeProjectScale(repositoryPath, repository) {
        try {
     ...
     */
    async getDirectoryStats(dirPath, depth = 0, maxDepth = 0) {
        let fileCount = 0;
        maxDepth = Math.max(maxDepth, depth);
        if (depth > 10)
            return { fileCount, maxDepth }; // Prevent infinite recursion
        try {
     ...
     */
    async generateAgentPrompts(repository) {
        const prompts = {};
...
Complexity: ${repository.complexity}
     ...
     */
    generateBackendPrompt(repository, baseContext) {
...
- ${hasTypeScript ? 'Use TypeScript for type safety' : 'Consider adding TypeScript for better maintainability'}`;
     ...
     */
    generateFrontendPrompt(repository, baseContext) {
        const packageManager = repository.packageManagers[0] || 'npm';
        const hasTypeScript = repository.techStack.includes('typescript');
        let prompt = `${baseContext}
...
- ${hasTypeScript ? 'Use TypeScript with strict type checking' : 'Consider adding TypeScript for component props'}`;
     ...
     */
    generateArchitectPrompt(repository, baseContext) {
        let prompt = `${baseContext}
...
- Balance simplicity with extensibility`;
     ...
     */
    getCachedResult(repositoryPath) {
     ...
     */
    setCachedResult(repositoryPath, result) {
     ...
     */
    getFallbackContext(repositoryPath) {
        return {
            repository: {
                path: repositoryPath,
                name: basename(repositoryPath),
     ...
     */
    async simulateSlowAnalysis(repositoryPath, delayMs) {
     ...
     */
    simulateDirectoryChange(directoryPath) {
        this.emit('directory_change', directoryPath);
    }
}
