/**
 * T015: Repository context switching integration test
 */
...
describe('Repository Context Switching Integration Tests', () => {
  ...
  it('should switch repository context and update agent prompts', async () => {
    // Start in first directory (React project)
    ...
    expect(context1?.fingerprint.techStack.dependencies).toContainEqual(
      expect.objectContaining({ name: 'react' })
    );
    
    // Switch to second directory (Express project)
    ...
    expect(context2?.fingerprint.techStack.dependencies).toContainEqual(
      expect.objectContaining({ name: 'express' })
    );
    
    // Agent prompts should be different
