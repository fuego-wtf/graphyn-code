...
describe('Approval Workflow Integration Tests', () => {
    ...
    it('should handle approval workflow with keyboard navigation', async () => {
        ...
        await orchestrator.handleKeyPress(' ', { ctrl: false, alt: false, shift: false, meta: false });
        await orchestrator.handleKeyPress('a', { ctrl: false, alt: false, shift: false, meta: false });
        const workflow = orchestrator.getCurrentWorkflow();
        expect(workflow?.status).toBe('approved');
    });
});
//# sourceMappingURL=approval-workflow.test.js.map